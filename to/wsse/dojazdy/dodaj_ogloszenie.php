...
$q_ile_ogl = "SELECT `id` FROM `wsse_ogloszenia` WHERE `kto` ='".$kto."'";

$b_ile_ogl = mysqli_query($con,$q_ile_ogl);

if(mysqli_num_rows($b_ile_ogl) < 3){
    
    $nach = mt_rand().mt_rand(2,time())."_wsse";

    
$q_dod_ogl = "INSERT INTO `wsse_ogloszenia` (`id`, `dodano`, `kto`, `zmiana`, `trasa`, `miejsc`, `czas`, `miejsce_pracy`, `dzien`, `dzien2`, `dzien3`, `dzien4`, `auto`, `tresc`, `znalazl`) VALUES ('".$nach."', NOW(), '".$kto."', '".$zmiana."', '".$trasa."', '".$miejsc."', '', '".$miejsce_pracy."', ".$dnibaza.", '".$auto."', '".$tresc."', '')";
