...
if(isset($_GET["jakazmiana"])){
    
    $jakazmiana = $_GET["jakazmiana"];
  ...
  /*  echo $jakazmiana." ".$filtrdni." ".$filtrtrasa." ".$_GET["filtrzmiana"];*/
   ...
   if($jakazmiana == "zmiana"){
        
        $zmianabaza = "zmiana";
        $zmianaget = $_GET["filtrzmiana"];
...
//domiejsc tez trzea cosrozkminic inaczej bo jeslijaszukam 1 miejsca a ktosblb ma 2 to mi go nie wyszuka 
            ...
            $zmiana = " WHERE (`".$zmianabaza."` RLIKE '".$zmianaget."+')";
          
            $przeddni = " AND ";
        }
        
    }else if($jakazmiana == "ak"){
    
        $zmianabaza = "ak";
        $zmianaget = $_GET["filtrak"];
 ...
 if($filtrtrasa == "tr_all"){
   ...
   $zapytanie = "SELECT * FROM `ogloszenia`".$zmiana.$dnibaza.$trasa.$miejsca." ORDER BY `dodano` DESC LIMIT ".$limit.",10";
   ...
   while($row = mysqli_fetch_assoc($baza)){
    ...
    $q_po_edycji = "SELECT * FROM `ogloszenia` WHERE `id`='".$id."'";
   ...
   while($row = mysqli_fetch_assoc($baza_po_edycji)){
    ...
    $q_po_edycji = "SELECT * FROM `archiwum` WHERE `id`='".$id."'";
...
//domiejsc tez trzea cosrozkminic inaczej bo jeslijaszukam 1 miejsca a ktosblb ma 2 to mi go nie wyszuka 
 ...
 if($filtrtrasa == "tr_all"){
   ...
   $zapytanie = "SELECT `id` FROM `ogloszenia`".$zmiana.$dnibaza.$trasa.$miejsca;
